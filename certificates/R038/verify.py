...
def need(ok, en, zh):
    if not ok: raise ValueError(f'{en} / {zh}')
...
def integrity():
    ...
    for name,path in actual.items():
        need((path.stat().st_size,sha256(path))==expected[name],f'manifest mismatch: {name}',f'清单哈希或字节数不匹配：{name}')
    return len(actual)

def recorded_checks():
    c=readj(ROOT/'CLAIMS.json');q=readj(ROOT/'results/R038_CONTAINMENT.json');b=readj(ROOT/'results/R038_BASE_CONTROL_CHUNKS.json');a=readj(ROOT/'results/R038_AUGMENTED_CONTROL_CHUNKS.json')
...
def merge_chunks(chunks):
    ...
    for z in chunks:
        minimum=z['minimum_units'] if minimum is None else min(minimum,z['minimum_units']);slabs+=z['center_slabs']
...
def full_geometry(certificate,out,base_record,augmented_record):
    ...
    for measure,record in [('base',base_record),('combined',augmented_record)]:
        ...
        for start,stop in RANGES:
            ...
            z=json.loads(p.stdout);chunks.append(z);frozen=next(x for x in record['chunks'] if x['range']==[start,stop])
...
def main():
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--certificate',type=Path,help='Pinned upstream certificate for full replay / 完整复演所需的锁定上游证书');ap.add_argument('--output',type=Path,required=True,help='New output directory / 新输出目录');args=ap.parse_args()
    need(not sys.flags.optimize,'optimized Python is not supported','不支持 Python 优化模式');need(not os.environ.get('PYTHONOPTIMIZE'),'PYTHONOPTIMIZE is not supported','不支持 PYTHONOPTIMIZE');need(not args.output.exists(),'output directory already exists','输出目录已经存在');args.output.mkdir(parents=True)
    try:
        c,q,b,a=recorded_checks();n=integrity();status=RECORDS
        if args.certificate is not None: full_geometry(args.certificate,args.output,b,a);status=FULL
        report={'status':status,'research_id':'N17-R038','claim':c['claim'],'strict_lower_bound':c['strict_lower_bound'],'strict_lower_bound_reduced':c['strict_lower_bound_reduced'],'integrity_files':n,'base_counting_surplus_units':b['counting_surplus_units'],'augmented_counting_surplus_units':a['counting_surplus_units'],'geometry_recomputed':args.certificate is not None,'scope':'Full status requires the SHA-pinned upstream certificate and all ten exact chunk scans; records-only status does not establish geometric coverage. / 完整状态要求 SHA 锁定的上游证书与十个精确分块扫描；仅账本状态不能建立几何覆盖。'}
        (args.output/'RESULT.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(status);return 0
    except BaseException as exc:
        fail={'status':'FAIL_OR_INCOMPLETE','error':f'{type(exc).__name__}: {exc}'};(args.output/'FAILURE.json').write_text(json.dumps(fail,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(f"FAIL_OR_INCOMPLETE: {exc}",file=sys.stderr);return 1
