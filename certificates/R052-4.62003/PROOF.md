...
## 点位延续 / Site continuation

从原R052的A0=230650/231001到本证书A1=461300/462003，令delta=A0−A1；最近墙坐标x改为x−delta/2，精确中线固定，另一侧由反射确定。D4组索引与资源权重保持，全部新几何最低值重新计算。 / From original R052 A0=230650/231001 to A1=461300/462003, set delta=A0−A1; replace nearest-wall coordinate x by x−delta/2, fix the exact midline and use reflection on the other side. D4 group indices and resource weights are retained, while all minima are recomputed for the new geometry.

## 缩放、资源与预算 / Scaling, resources and budget

假设端点 462003/100000 可装填；按 A=461300/462003 缩放后，十七个边长 A 的父方形位于边长 L=4613/1000 的容器中。 / Suppose packing is possible at 462003/100000; scaling by A=461300/462003 yields seventeen parent squares of side A in a container of side L=4613/1000.
...
15727 个连续区间 [a,b] 无空隙覆盖 0 至 207107/500000；末端满足 u²+2u−1=309449/250000000000>0，故覆盖 tan(π/8)。每行 [a,b,t,B] 给出同中心、方向 t、边长 B 的闭内核。 / The 15727 contiguous intervals [a,b] cover 0 through 207107/500000 without gaps; the last endpoint satisfies u²+2u−1=309449/250000000000>0 and therefore covers tan(π/8). Each row [a,b,t,B] specifies a concentric closed core of orientation t and side B.

独立有理检查核对全部 62908 条端点轴向投影不等式，最小严格余量为 4613/9240060000000000000。对核心与父方向之差 δ，其范围满足 |δ|≤π/4，投影因子 cosδ+|sinδ| 随 |δ| 单调增加，因此端点检查推出整个区间的严格内含。 / Independent rational checks establish all 62908 endpoint axial-projection inequalities, with minimum strict margin 4613/9240060000000000000. The core-to-parent angle difference satisfies |δ|≤π/4, where cosδ+|sinδ| increases with |δ|, so endpoint checks imply strict containment throughout each interval.
...
完整独立账本覆盖行 0 至 15726，每行恰一次；最低收费为 999426274093，仅第15560、15561与15562行达到。123 个原始分块及逐行账本随包提供；完整复演必须覆盖全部行。 / The complete independent ledger covers rows 0 through 15726 exactly once; its minimum is 999426274093, attained only by rows 15560,15561 and15562. All 123 original blocks and the per-row ledger are included; full replay must cover every row.
...
最小装填边长可达到：取可行边长趋向下确界的有界序列，中心与方向处于紧集，收敛子列保留容器包含关系；若极限中两方形内部相交，正余量会使充分靠后的两方形也相交，矛盾。因此端点被排除推出严格结论 s(17)>4.62003。 / The minimum feasible side is attained: take a bounded sequence of feasible sides approaching its infimum, with centres and orientations in a compact set. A convergent subsequence preserves containment; any interior overlap in the limit would have positive slack and persist in sufficiently late packings, a contradiction. Excluding the endpoint therefore yields the strict conclusion s(17)>4.62003.
