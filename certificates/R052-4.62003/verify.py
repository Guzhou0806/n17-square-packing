#!/usr/bin/env python3
"""R052 continuation exact public replay / R052 延续精确公开复演。"""
...
CERT_SHA='74d884cbb1f9ca0c9684bb940ecd13781db53e6efe7a775d852e15ba618aafe3'
...
def block_check(rec,lo,hi,expected):
    ...
    need(rec['status']=='PASS_EXACT_MOVABLE_SUPPORT_SCAN' and rec['certificate_sha256']==CERT_SHA and rec['parent_side']=='461300/462003' and rec['measure']=='combined','Block identity/status')
...
def records():
    ...
    cert=json.loads(raw)
    need(cert['L']=='4613/1000' and cert['A']=='461300/462003' and Fraction(cert['L'])/Fraction(cert['A'])==Fraction(462003,100000),'Bound identity')
...
def containment(cert):
    ...
    need(result['minimum_containment_margin']=='4613/9240060000000000000','Containment margin')
...
def main():
    ...
    result={'status':'PASS_R052_'+mode.upper().replace('-','_'),'mode':mode,'bound':'462003/100000','candidate_sha256':CERT_SHA,'manifest_sha256':manifest,'checked_rows':N if rows is not None else 0,'archived_rows':N,'minimum_units':MINIMUM,'budget_units':BUDGET,'strict_surplus_units':2,'structure':structure,'seconds':time.monotonic()-started,'scope':'Fresh full centre replay only in full modes / 仅 full 模式重新计算完整中心空间'}
