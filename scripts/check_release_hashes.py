...
def main():
    try:
        legacy=verify.read_json(ROOT/'RELEASE_MANIFEST.json');r012=verify.read_json(ROOT/'R012_PUBLICATION.json');r038=verify.read_json(ROOT/'R038_PUBLICATION.json')
        expected=dict(legacy['files']);expected.update(r012['files']);expected.update(r038['files'])
        ...
        print(f'FAIL_RELEASE_BYTES: {exc} / 发布字节检查失败',file=sys.stderr);return 1
